...
class homekit_integration : public esp32::singleton<homekit_integration>
{
  public:
    void begin();
  ...
  private:
    homekit_integration(config &config, hardware &hardware) : config_(config), hardware_(hardware), homekit_task_([this] { homekit_task_ftn(); })
    {
    }
    friend class esp32::singleton<homekit_integration>;
